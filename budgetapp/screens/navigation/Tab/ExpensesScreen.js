...
const ExpensesScreen = () => {
  ...
  const totalSpending = filteredTransactions.reduce((acc, cur) => acc + Math.abs(cur.amount), 0);
  ...
  const pieChartData = Object.keys(categoryData).map((category, index) => ({
    ...
    percentage: ((categoryData[category] / totalSpending) * 100).toFixed(2),
  ...
  return (
    <View style={styles.container}>
      <Text style={{ fontSize: 20, color: '#5F71D6' }}>Total Spending: ${totalSpending.toFixed(2)}</Text>
